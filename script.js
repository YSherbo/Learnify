...
var AI_Request = "Can you give me a step by step Roadmap for learning " + User_Input + " for beginners and with no Context At All and no explanation and give one recommendation (like when giving step just say Django instead of NoSql : Django for example) and no bold text and make sure the steps doesn’t exceed 13 steps";//Request Sent To The AI Model
...
app.post('/', (req, res) => {
  const { parcel } = req.body
  User_Input = parcel
  AI_Request = "Can you give me a step by step Roadmap for learning " + User_Input + " for beginners and with no Context At All and no explanation and give one recommendation (like when giving step just say Django instead of NoSql : Django for example) and no bold text and make sure the steps doesn’t exceed 13 steps";
  console.log(parcel)
  if(!parcel) {
    return res.status(400).send({ info: 'err' })
  }
