...
var AI_Request = "Can you give me a step by step Roadmap for learning " + User_Input + " for beginners and with no Context At All and no explanation and if steps have a tool, Programing language, software..etc (like exploitation need a tool like metasploit), give one recommendation (like when giving step just say Django instead of NoSql : Django for example or Vulnerabilities and Exploits : metasploit just say metasploit ) and no bold text and make sure the steps doesn’t exceed 13 steps and with no certification or exam or competitions or programs ( no CEH Exem ...etc) and no sections (no FUNDAMENTALS : networking basics... just say networking)";//Request Sent To The AI Model
